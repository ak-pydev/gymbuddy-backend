#!/usr/bin/env python3
"""
Run gating sweep and generate risk-coverage curve for gym_2d experiments.
This is a dedicated version for the gym_2d pipeline.
"""
...
def run_sweep():
    ...
    parser.add_argument('--out_dir', type=str, required=True, help='Output directory for all artifacts')
    ...
    # Define output paths automatically
    csv_path = os.path.join(out_dir, "gating.csv")
    fig_path = os.path.join(out_dir, "gating.png")
    ...
    safety_json_path = os.path.join(out_dir, "safety_table.json")
    ...
    print("\n" + "="*70)
    print(f"SAFETY METRICS TABLE - {dataset_name}")
    print("="*70)
    ...
    plt.title(f"Risk-Coverage Curve - {dataset_name}", fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.xlim(0, 1)
    if risks and wrong_spoke_rates:
    ...
    plt.title(f"Threshold Sensitivity - {dataset_name}", fontsize=14)
    fig.tight_layout()
    
    threshold_fig_path = os.path.join(out_dir, "threshold_sensitivity.png")
